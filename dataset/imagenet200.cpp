...
int train() {
	...
	net_utils::readBytes(in_images, train_images, total_size * data_dim);
	in_images.close();

	uint8_t* train_labels = new uint8_t[total_size * label_dim];
	ifstream in_labels(root_path + "labels.bin");
	net_utils::readBytes(in_labels, train_labels, total_size * label_dim);
	in_labels.close();
	...
	network.PushConvolution(48, 5, 1, 0, -6e-2f, 0.02f, 0.9f, 0.001f);
	network.PushActivation(CUDNN_ACTIVATION_RELU);
	network.PushPooling(2, 2);
	network.PushConvolution(64, 5, 1, 0, -6e-2f, 0.02f, 0.9f, 0.001f);
	network.PushActivation(CUDNN_ACTIVATION_RELU);
	network.PushPooling(2, 2);
	network.PushConvolution(64, 4, 1, 0, -6e-2f, 0.02f, 0.9f, 0.001f);
	network.PushActivation(CUDNN_ACTIVATION_RELU);
	network.PushPooling(2, 2);
